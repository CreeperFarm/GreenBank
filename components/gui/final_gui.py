...
def final_gui(borrowing_rate_percentage, borrowing_rate, bonus_passenger):
    ...
    if bonus_passenger < 0:
        ...
        ttk.Label(window_final, text="Le bonus avec le nombre de passagers est : " + bonus_passenger + "%").grid(row=0, column=0)
    # Show the malus passenger
    else:
        ttk.Label(window_final, text="Le malus avec le nombre de passagers est : " + str(bonus_passenger) + "%").grid(row=0, column=0)

    # Show the borrowing rate
    ttk.Label(window_final, text="Le taux d'emprunt avec le critère de la voiture est : " + str(borrowing_rate) + "%").grid(row=1, column=0)

    # Show the borrowing rate total
    ttk.Label(window_final, text="Le taux d'emprunt est de : " + str(borrowing_rate_percentage) + "%").grid(row=2, column=0)
